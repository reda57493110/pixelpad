...
const PRODUCTS_CACHE_DURATION = 60 * 60 * 1000 // 60 minutes (1 hour - maximum cache for fastest reloads)
...
export async function getAllProducts(bypassCache: boolean = false): Promise<Product[]> {
  try {
    ...
    const response = await fetch(API_BASE, {
      ...
      next: bypassCache ? { revalidate: 0 } : { revalidate: 1800 } // 30 minutes revalidation (increased for faster reloads)
    })
    if (!response.ok) {
...
const CACHE_DURATION = 60 * 60 * 1000 // 60 minutes (1 hour - maximum cache for fastest reloads)
...
export async function getProductById(id: string): Promise<Product | undefined> {
  try {
    ...
    const response = await fetch(`${API_BASE}/${id}`, {
      cache: 'force-cache',
      next: { revalidate: 1800 } // 30 minutes revalidation (increased for faster reloads)
    })
...
export async function getProductPageProducts(): Promise<Product[]> {
  try {
    // Check cache first
