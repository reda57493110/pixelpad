...
export default function RegisterForm({ onClose, onSwitchToLogin }: RegisterFormProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (success) {
        ...
        setError('Email already exists. Please use a different email or try signing in.')
      }
    } catch (error) {
  ...
  return (
    <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4">
      <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-2xl w-full max-w-md animate-in slide-in-from-bottom duration-300">
        {/* Header */}
